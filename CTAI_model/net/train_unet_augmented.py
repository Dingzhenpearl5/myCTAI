"""
UNet+AttentionGate 训练脚本 - 数据增强版本
专门用于测试数据增强效果,独立于原训练脚本
"""
...
class Config:
    ...
    model_name = 'unet_augmented_medium'  # 区分不同的增强版本
...
print("="*60)
print("UNet+AttentionGate 训练 - 数据增强版本")
print("="*60)
...
print(f"数据增强模式: {config.augmentation_mode}")
print("="*60)
...
# 增强详情
print("\n数据增强策略:")
if config.augmentation_mode == 'light':
    print("  - 水平翻转 (p=0.5)")
    print("  - 旋转±10° (p=0.3)")
    print("  - 亮度/对比度调整 0.9-1.1 (p=0.3)")
elif config.augmentation_mode == 'medium':
    print("  - 水平翻转 (p=0.5)")
    print("  - 垂直翻转 (p=0.3)")
    print("  - 旋转±15° (p=0.4)")
    print("  - 平移±5% (p=0.3)")
    print("  - 亮度/对比度调整 0.8-1.2 (p=0.5)")
    print("  - 高斯噪声 (p=0.2)")
    print("  - 高斯模糊 (p=0.15)")
elif config.augmentation_mode == 'heavy':
    print("  - 水平翻转 (p=0.5)")
    print("  - 垂直翻转 (p=0.5)")
    print("  - 旋转±20° (p=0.5)")
    print("  - 平移±10% (p=0.4)")
    print("  - 弹性形变 (p=0.3)")
    print("  - 亮度/对比度调整 0.7-1.3 (p=0.6)")
    print("  - 高斯噪声 (p=0.3)")
    print("  - 高斯模糊 (p=0.2)")
...
with open(report_path, 'w', encoding='utf-8') as f:
    f.write("="*60 + "\n")
    f.write("UNet+AttentionGate 训练报告 - 数据增强版本\n")
    f.write("="*60 + "\n\n")
    ...
    if config.augmentation_mode == 'light':
        f.write("- 水平翻转 (p=0.5)\n")
        f.write("- 旋转±10° (p=0.3)\n")
        f.write("- 亮度/对比度调整 0.9-1.1 (p=0.3)\n")
    elif config.augmentation_mode == 'medium':
        f.write("- 水平翻转 (p=0.5)\n")
        f.write("- 垂直翻转 (p=0.3)\n")
        f.write("- 旋转±15° (p=0.4)\n")
        f.write("- 平移±5% (p=0.3)\n")
        f.write("- 亮度/对比度调整 0.8-1.2 (p=0.5)\n")
        f.write("- 高斯噪声 (p=0.2)\n")
        f.write("- 高斯模糊 (p=0.15)\n")
    elif config.augmentation_mode == 'heavy':
        f.write("- 水平翻转 (p=0.5)\n")
        ...
        f.write("- 平移±10% (p=0.4)\n")
        f.write("- 弹性形变 (p=0.3)\n")
        f.write("- 亮度/对比度调整 0.7-1.3 (p=0.6)\n")
        f.write("- 高斯噪声 (p=0.3)\n")
        f.write("- 高斯模糊 (p=0.2)\n")
    f.write("\n")
    ...
    f.write("## 对比基线模型\n")
    f.write("- 基线测试Dice: 0.7802 (78.02%, 无数据增强)\n")
    f.write(f"- 增强版测试Dice: {test_dice:.4f} ({test_dice*100:.2f}%)\n")
    if test_dice > 0.7802:
        improvement = (test_dice - 0.7802) * 100
        f.write(f"- 提升: +{improvement:.2f}% ✅\n")
    else:
        decrease = (0.7802 - test_dice) * 100
        f.write(f"- 下降: -{decrease:.2f}% ⚠️\n")
    f.write("\n")
...
print("与基线模型对比")
print("="*60)
baseline_dice = 0.7802
print(f"基线模型 (无增强): {baseline_dice:.4f} ({baseline_dice*100:.2f}%)")
print(f"增强模型 ({config.augmentation_mode}): {test_dice:.4f} ({test_dice*100:.2f}%)")
if test_dice > baseline_dice:
    improvement = (test_dice - baseline_dice) * 100
    print(f"提升: +{improvement:.2f}% ✅")
else:
    decrease = (baseline_dice - test_dice) * 100
    print(f"下降: -{decrease:.2f}% (可能需要调整增强参数)")
print("="*60)
